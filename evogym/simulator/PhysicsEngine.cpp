...
// CHANGED TO RK2 (but keeping function name)
...
void PhysicsEngine::resolve_edge_constraints() {
    ...
    const double thresh_diff = 0.25;  // applied in both directions simultaneously
    const double correction_factor = 0.8;

    // Compute vector difference and current distance
    Array <double, 2, Dynamic> vec_q_to_p = (*pos)(Eigen::all, *a_index) - (*pos)(Eigen::all, *b_index);
    Array <double, 1, Dynamic> dist = (vec_q_to_p.row(0).square() + vec_q_to_p.row(1).square()).sqrt();

    // Calculate stress (ratio of current to rest length)
    Array <double, 1, Dynamic> stress = dist / init_length_eq->array();

    // Check if correction is needed (outside tolerance)
    Array <bool, 1, Dynamic> needs_correction = (stress > (1 + thresh_diff)) || (stress < (1 - thresh_diff));

    // Calculate correction magnitude to bring back to rest length
    Array <double, 1, Dynamic> correction_mag = needs_correction.select(
        (dist - init_length_eq->array()) / (dist * 2),  // Half the error, to be split between points
        0.0 // No correction needed if needs_correction is false
    );

    // Apply correction to direction vector
    Array <double, 2, Dynamic> correction = vec_q_to_p;
    correction.row(0) *= correction_mag;
    correction.row(1) *= correction_mag;
    ...
    // Apply corrections
    (*pos)(Eigen::all, *a_index) -= a_fixed.select(0.0, correction.matrix() * correction_factor);
    (*pos)(Eigen::all, *b_index) += b_fixed.select(0.0, correction.matrix() * correction_factor);
}
